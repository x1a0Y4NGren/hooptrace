...
import 'package:hooptrace/core/data/migrations.dart';
...
void main() {
  test('domain contract exposes the 1.0 enum vocabulary', () {
    expect(RecordingMode.values, containsAll([RecordingMode.simple, RecordingMode.detailed]));
    expect(MatchLifecycle.values, containsAll([MatchLifecycle.active, MatchLifecycle.finished, MatchLifecycle.archived]));
    expect(ClockMode.values, containsAll([ClockMode.countUp, ClockMode.countdown]));
    expect(ClockPhase.values, containsAll([ClockPhase.regulation, ClockPhase.regulationExpired, ClockPhase.overtime]));
    expect(EventKind.values, containsAll([EventKind.fieldGoal, EventKind.freeThrow, EventKind.custom]));
    expect(ShotOutcome.values, containsAll([ShotOutcome.made, ShotOutcome.missed]));
    expect(PossessionPolicy.values, containsAll([PossessionPolicy.manual, PossessionPolicy.switchAfterMade, PossessionPolicy.keepAfterMade]));
    expect(RestoreMode.values, containsAll([RestoreMode.replace, RestoreMode.merge]));
    expect(ThemePreference.values, containsAll([ThemePreference.system, ThemePreference.light, ThemePreference.dark]));
    expect(TrackingCoverage.values, containsAll([TrackingCoverage.scoresOnly, TrackingCoverage.shotAttempts]));
  });

  test('domain snapshots participant ownership and normalizes custom labels', () {
  ...
  test('fresh schema has v2 tables, foreign keys, indexes and integrity', () async {
  ...
  test('participant and event graph enforces one field-goal location and cascades', () async {
    final database = createTestDatabase();
    ...
    await database.into(database.matches).insert(
          MatchesCompanion.insert(
            id: 'match-1',
            status: MatchLifecycle.active.name,
            ruleTemplateJson: '{}',
            createdAt: DateTime.utc(2026),
          ),
        );
    await database.into(database.matchParticipants).insert(
    ...
    expect(
      database.into(database.shotLocations).insert(
        ShotLocationsCompanion.insert(
          id: 'location-2',
          matchId: 'match-1',
          eventId: 'field-goal',
    ...
    await database.into(database.matchClocks).insert(
          MatchClocksCompanion.insert(
            id: 'clock-1',
            matchId: 'match-1',
            mode: ClockMode.countUp.name,
            phase: ClockPhase.regulation.name,
            accumulatedSeconds: const Value(12),
          ),
        );
    await database.into(database.activeSessions).insert(
    ...
    await (database.delete(database.matches)..where((row) => row.id.equals('match-1'))).go();
  ...
  test('schema v1 is rejected before a migration can modify the database', () async {
    final database = AppDatabase.inMemory();
    addTearDown(database.close);
    await database.customStatement('PRAGMA user_version = 1');

    await expectLater(
      database.select(database.matches).get(),
